...
function saveCart() {
  // Any change to the cart invalidates a previously placed order — the
  // receipt no longer matches what's in the cart until they order again.
  orderPlaced = false;
...
function renderCart() {
  ...
  // "GST Receipt" only unlocks once an order has actually been placed
  // via "Order Now" — it's not enough to just have items in the cart.
  viewReceiptBtn.disabled = !orderPlaced || cart.length === 0;
...
function setupEventListeners() {
  ...
  if (checkoutReceiptBtn) checkoutReceiptBtn.onclick = placeOrder;
  ...
  if (downloadReceiptBtn) downloadReceiptBtn.onclick = downloadReceiptAsPDF;
}

// Download the printable invoice as a PDF file
function downloadReceiptAsPDF() {
  const invoiceEl = document.getElementById('printableInvoice');
  if (!invoiceEl) return;

  if (typeof html2pdf === 'undefined') {
    // Fallback if the PDF library failed to load (e.g. offline) — print
    // dialog lets the user "Save as PDF" instead.
    window.print();
    return;
  }

  const invNumText = document.getElementById('invNum')?.textContent?.trim() || 'invoice';
  const originalLabel = downloadReceiptBtn ? downloadReceiptBtn.innerHTML : null;
  if (downloadReceiptBtn) {
    downloadReceiptBtn.disabled = true;
    downloadReceiptBtn.innerHTML = '<span>Preparing…</span>';
  }

  const opts = {
    margin: 10,
    filename: `BlinkList-${invNumText}.pdf`,
    image: { type: 'jpeg', quality: 0.98 },
    html2canvas: { scale: 2, backgroundColor: '#ffffff' },
    jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }
  };

  html2pdf().set(opts).from(invoiceEl).save().then(() => {
    if (downloadReceiptBtn && originalLabel) {
      downloadReceiptBtn.disabled = false;
      downloadReceiptBtn.innerHTML = originalLabel;
    }
  }).catch(() => {
    if (downloadReceiptBtn && originalLabel) {
      downloadReceiptBtn.disabled = false;
...
// Order Now — places the order, which unlocks the GST Receipt button,
// then immediately shows the invoice for the order just placed.
function placeOrder() {
  if (cart.length === 0) return;
  orderPlaced = true;
  renderCart();
...
function openReceiptModal() {
  if (cart.length === 0 || !receiptModal || !orderPlaced) return;
  ...
  if (document.getElementById('invGrandTotal')) document.getElementById('invGrandTotal').textContent = `₹${grandTotal.toFixed(2)}`;
